...
function oppKeys(o){
 const out=new Map();
 for(const [mode,m] of Object.entries(o?.modes||{})) for(const x of m.opportunities||[]) out.set(`${mode}|${x.role}|${x.from}|${x.to}|${x.classification}`,x);
 return out;
}
...
function buildReport(oldC,newC){
 ...
 for(const n of [...a.keys()].filter(n=>b.has(n)).sort()){
   ...
   if(parts.length)priceChanges.push({name:n,text:`• ${n}: ${parts.join('; ')}`});
 }
 ...
 const material=added.length||removed.length||priceChanges.length||roleChanges.length||autoSafe.length||reviewOnly.length;
 if(!material)return {report:'NO_REPLY',oldD,newD};
 const lines=['🧭 Octopus benchmark — revisão semanal',`Snapshot: ${oldD.date||oldC.slice(0,7)} → ${newD.date||newC.slice(0,7)}`,''];
 if(added.length||removed.length){lines.push('MODELOS');lines.push(...fmtModelList('Entraram:',added,'+'));lines.push(...fmtModelList('Saíram:',removed,'-'));lines.push('')}
 ...
 if(autoSafe.length){lines.push('• Trocas auto-safe novas para rever:');for(const x of autoSafe.slice(0,5))lines.push(`  - ${x.role}: ${x.from} → ${x.to} (${x.metric})`)}
 if(reviewOnly.length){lines.push('• Novas candidatas review-only:');for(const x of reviewOnly)lines.push(`  - ${x.role}: ${x.from} → ${x.to}`)}
