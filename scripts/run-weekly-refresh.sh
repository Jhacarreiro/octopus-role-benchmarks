...
write_partial() {
  ...
  local message="${2:-}"
  if [ -n "$message" ]; then
    node scripts/write-refresh-status.mjs --status partial --mode "$mode" --message "$message" --run-url "$RUN_URL"
  else
...
if [ "$update_code" -ne 0 ]; then
  if grep -q "SciCode estimator validation failed:" /tmp/octopus-refresh.log; then
    ...
    write_partial "lineup-inputs-only" "Full refresh blocked by SciCode estimator validation; refreshed CommandCode Max pricing/allowances, AA Intelligence, CyberBench and portfolios from the last validated benchmark snapshot."
    exit 0
  fi
